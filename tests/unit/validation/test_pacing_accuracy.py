"""Validation tests for real-time audio pacing accuracy.

Goal: "a 1m30s clip should take 1m30s to send out". These tests codify the
pacing-drift findings:
...
def test_drifting_sleep_is_detectably_worse_than_deadline():
    ...
    # deadline pacing should be close to real-time, and the drifting loop should
    # overshoot by a clear multiple. Compare the OVERSHOOT (ratio - 1) rather
    # than the ratio: how far a drifting loop runs long is set by the host's
    # per-sleep overhead, which varies by an order of magnitude across platforms,
    # but the drifting loop accumulates it every chunk while the deadline loop
    # absorbs it — so the ratio between their overshoots stays large everywhere.
    assert deadline < 1.10
    drift_excess = drift - 1.0
    deadline_excess = max(deadline - 1.0, 1e-4)  # floor: avoid dividing by noise
    assert drift_excess > 3.0 * deadline_excess, (
        f"drifting sleep overshot real-time by {100*drift_excess:.3f}% but "
        f"deadline pacing by {100*deadline_excess:.3f}% — the drifting loop "
        f"should accumulate several times the error"
    )
