...
runs in CI; locally it needs transformers importable (or a stub on PYTHONPATH).
"""
...
def test_stt_end_to_end_produces_transcript_and_metrics(tmp_path: Path):
    """STT responses flow through the real pipeline into the ASR scoring path.

    Every STT request carries ground truth (the audio trace generator enforces
    this at load time); the evaluator scores WER + stream latencies and never
    routes STT rows through the TTS timing path.
    """
    ...
    try:
        ...
        for sid in range(n):
            sched.schedule_session(
                _stt_session(sid, wav, expected_transcript="the quick brown fox")
            )
        ...
        # perfect transcripts -> zero WER
        assert math.isclose(m["asr_final_corpus_wer"], 0.0, abs_tol=1e-9)
        # realtime stream latency distributions surfaced
        assert any(k.startswith("Time to First Visible Text") for k in m)
        assert any(k.startswith("Time to Final Transcript") for k in m)
...
def test_stt_end_to_end_scores_wer_when_ground_truth_present(tmp_path: Path):
    ...
    Ground-truth-carrying STT requests flow
