...
@pytest.mark.unit
def test_build_ws_url_shared_helper() -> None:
    from veeksha.client.utils import build_ws_url

    assert build_ws_url("http://h:9000", "/v1/realtime") == "ws://h:9000/v1/realtime"
    assert build_ws_url("https://h", "/p") == "wss://h/p"
    assert build_ws_url("ws://h", "/p") == "ws://h/p"  # non-http passthrough
    assert build_ws_url("", "/p") == "/p"


@pytest.mark.unit
def test_stt_client_ws_url_uses_shared_helper() -> None:
    client = _vajra_realtime_client()
    assert (
        client._ws_url == "ws://localhost:8003/openai/v1/realtime?intent=transcription"
    )
