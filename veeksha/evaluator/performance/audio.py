...
ABC. This is the modality-unification payoff — audio and text
...
from veeksha.core import cross_stream
...
class AudioPerformanceEvaluator(BaseChannelPerformanceEvaluator):
    ...
    def __init__(
        ...
        self.summaries: Dict[str, ShardedCDFSketch] = {
            ...
            # TTS interactivity: alignment of the paced input-text timeline
            # against the audio-delta timeline (realtime dialect only).
            "first_input_to_first_audio": ShardedCDFSketch(
                "First Input to First Audio", unit="s"
            ),
            "voice_response_latency": ShardedCDFSketch(
                "Voice Response Latency", unit="s"
            ),
    ...
    def register_request(
        ...
        """No-op: every audio metric is derived from the completed response.

        Kept because the channel-evaluator ABC requires the hook.
        """
        return None
    ...
    def record_request_completed(
        ...
        # STT responses must NEVER fall through to the TTS timing path.
        if metrics.get(ac.AUDIO_TASK) == AudioTask.STT:
            if (
                metrics.get("final_transcript") is None
                or metrics.get("expected_transcript") is None
            ):
                logger.warning(
                    "STT response for request %d is missing final_transcript/"
                    "expected_transcript; skipping ASR scoring.",
                    request_id,
                )
                return
            try:
                ...
                # One malformed row must not kill the completion thread.
                logger.warning(
                    "Failed to score STT response for request %d; skipping.",
                    request_id,
                    exc_info=True,
                )
                return
    ...
    def _compute_metrics(
        ...
        if ac.AUDIO_CHUNK_TIMESTAMPS in metrics:
            ...
            # TTS interactivity (streaming input -> streaming output), via the
            # shared cross-stream operators over the paced text-delta timeline:
            #   first_input_to_first_audio — how soon speech starts after the
            #     first text delta went in;
            #   response_latency — last text delta -> first audio (the general
            #     turn-latency operator; negative = audio overlapped input).
            text_deltas = metrics.get(ac.AudioMetricKey.TEXT_DELTA_TIMESTAMPS) or []
            if text_deltas:
                input_stream = TimedEventStream(
                    ChannelModality.TEXT,
                    [
                        StreamEvent(
                            offset_s=float(row[0]) / 1000.0,
                            size=int(row[1]),
                            kind="input",
                        )
                        for row in text_deltas
                    ],
                )
                first_in = input_stream.time_to_first_event()
                first_audio = stream.time_to_first_event()
                if first_in is not None and first_audio is not None:
                    derived["first_input_to_first_audio"] = first_audio - first_in
                derived["voice_response_latency"] = cross_stream.response_latency(
                    input_stream, stream
                )
            return derived
