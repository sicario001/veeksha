...
from veeksha.client.utils import build_ws_url
...
def _audio_to_pcm16_bytes(audio_path: str, target_sr: int) -> bytes:
    ...
    Uses soundfile to decode and scipy to resample rather than librosa, which
    pulls in numba — numba has no free-threaded build, so importing it here
    would make this client uninstallable on the very Python veeksha requires.
    """
    import soundfile as sf

    audio, source_sr = sf.read(audio_path, dtype="float32", always_2d=True)
    ...
    if source_sr != target_sr:
        ...
        audio = resample_poly(
            audio, int(target_sr) // divisor, int(source_sr) // divisor
        )

    # clip before scaling: resampling can overshoot slightly past [-1, 1], which
    # would wrap around to the opposite sign when cast to int16.
    ...
    *,
    ...
    if end_sample > total_samples:
        raise ValueError(
            ...
            f"end_ms={end_ms}, "
            f"clip_ms={pcm_bytes_to_duration_ms(len(pcm_bytes), sample_rate):.3f}"
        )
...
class STTStreamResult:
    ...
    chunk_count: int
    pcm_byte_count: int
    # Client-side dispatch stamp of each paced audio append (ms from the pacing
    # origin). Correlated against server-observed arrivals, this separates the
    # client's own pacing error from the network/server-receive floor.
    send_offsets_ms: list[float] = dataclass_field(default_factory=list)
...
class _STTClientBase(BaseLLMClient):
    ...
    def __init__(self, config: "STTClientConfig", **kwargs) -> None:
        ...
        self._ws_url = build_ws_url(self.api_base or "", self.ws_path)
        ...
        async with websockets.connect(
            ...
            async def _send() -> None:
                ...
                for byte_offset in range(0, len(pcm_bytes), self._ws_chunk_size):
                    ...
                    # CLIENT-side dispatch stamp: when this chunk's send was
                    # issued, relative to the pacing origin. Correlating these
                    # against server-observed arrivals separates client pacing
                    # error from the network/server-receive floor.
                    send_offsets_ms.append((time.monotonic() - audio_started_at) * 1000)
            ...
            try:
                while True:
                    raw = await ws.recv()
                    # stamp on arrival, before parsing, so our own decode cost
                    # is not folded into the recorded arrival time
                    now = time.monotonic()
    ...
    async def send_request(
        ...
        if success and stream_result is not None:
            input_audio_duration_ms = pcm_bytes_to_duration_ms(
            ...
            metrics_dict: dict = {
                ...
                "send_offsets_ms": stream_result.send_offsets_ms,
            }
