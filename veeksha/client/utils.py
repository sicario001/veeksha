...
def build_ws_url(api_base: str, path: str = "") -> str:
    """Convert an http(s) ``api_base`` to its ws(s) equivalent, appending ``path``.

    Non-http(s) bases pass through unchanged (already ws:// or a bare host).
    """
    base = api_base or ""
