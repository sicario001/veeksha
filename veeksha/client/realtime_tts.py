...
from veeksha.client.utils import TextDeltaPacer, build_ws_url, segment_text
...
class RealtimeTTSProtocol:
    ...
    def build_ws_url(self, api_base: str) -> str:
        normalized_base = build_ws_url(api_base).rstrip("/")
...
class RealtimeTTSClient(BaseLLMClient):
    ...
    async def send_request(
        ...
        # (Tracker liveness on error paths is owned by the client_runner, which
        # advances the DispatchTracker unconditionally after send_request
        # returns — no per-client fallback firing needed here.)
